...
test.describe("Homepage", () => {
  ...
  test("should display trending movies section", async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="trending-section"], .carousel, .trending',
      { timeout: 10000 }
    );

    // Check if trending movies are displayed
    const trendingSection = page
      .locator('.carousel, .trending, [data-testid="trending"]')
      .first();
    await expect(trendingSection).toBeVisible();

    // Check if movie cards are present
    await page.waitForSelector('.movieCard, [data-testid="movie-card"]', {
      timeout: 10000,
    });
